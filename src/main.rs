...
mod rustychess {
	...
	pub enum Player { White, Black, None }
	...
	impl Chessboard {
		pub fn new(player: Player) -> Chessboard {
			...
			Chessboard { fields: fields, perspective: player }
		}
	}
	...
	fn map_field(field: &Field) -> &'static str {
		match field.token {
			Token::None =>   " ",
			Token::Pawn =>   if field.player == Player::White { "\u{2659}" } else { "\u{265F}" },
			Token::Rook =>   if field.player == Player::White { "\u{2656}" } else { "\u{265C}" },
			Token::Knight => if field.player == Player::White { "\u{2658}" } else { "\u{265E}" },
			Token::Bishop => if field.player == Player::White { "\u{2657}" } else { "\u{265D}" },
			Token::King =>   if field.player == Player::White { "\u{2654}" } else { "\u{265A}" },
			Token::Queen =>  if field.player == Player::White { "\u{2655}" } else { "\u{265B}" }
		}
	}
